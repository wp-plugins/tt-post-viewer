...
add_action( 'widgets_init', 'initialize_category_widget' );
function initialize_category_widget() {
        global $opt_val;

        if($opt_val['ttpv_chk_category_widget'] == 'on'){
            register_widget( 'TTPV_Category_Widget' );
        }
}

class TTPV_Category_Widget extends WP_Widget {
	function __construct(){
	$options = array(
		'description' => "TTPV Widget: Display Category Posts",
		'name' => 'TTPV: Category Posts'
		);
	parent::__construct('TTPV_Category_Widget','',$options);
	}

	public function form($instance){
                global $opt_val;
		$defaults = array( 'title' => $opt_val['ttpv_txt_category_title'],
                                  'num_post' => $opt_val['ttpv_txt_category_widget_number'],
                                  'displaytype' => $opt_val['ttpv_drp_category_widget_postdisplay'],
                                  'categorylist' => implode(',', $opt_val['ttpv_drp_category_widget_category'])
                                  );
		...
		?>
                ...
                <p>
                    ...
                    value="<?php if(isset($title)){
                                    ...
                                    echo esc_attr($opt_val['ttpv_txt_category_title']);
                                }
                                ?>"  />
                </p>
			...
			<select id="<?php echo $this->get_field_id( 'displaytype' ); ?>" name="<?php echo $this->get_field_name( 'displaytype' ); ?>">
                ...
                <p>
                    ...
                    value="<?php if(isset($num_post)){
                                    ...
                                    echo esc_attr($opt_val['ttpv_txt_category_widget_number']);
                                }
                                ?>"  />
                </p>
                <?php	    
		...
		?>
		    ...
		    if($categories[0]=="Any"){
                        $parameters = array( 'posts_per_page' => $num_post, 'orderby' => 'title', 'order' => 'DESC', 'ignore_sticky_posts' => 1 );
		    }
                    else{			
                        $parameters = array( 'posts_per_page' => $num_post, 'category__in' => $catid, 'orderby' => 'title', 'order' => 'DESC', 'ignore_sticky_posts' => 1 );
                    }
