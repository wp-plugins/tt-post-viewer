...
add_action( 'widgets_init', 'initialize_recentpost_widget' );

function initialize_recentpost_widget() {
        global $opt_val;

        $opt_val = TT_PostViewer::get_option_values();
        if($opt_val['ttpv_chk_recentpost_widget'] == 'on'){
            register_widget( 'TTPV_Recentpost_Widget' );
        }
}

class TTPV_Recentpost_Widget extends WP_Widget {
	function __construct(){
	$options = array(
		'description' => "TTPV Widget: Display Recent Posts",
		'name' => 'TTPV: Recent Posts'
		);
	parent::__construct('TTPV_Recentpost_Widget','',$options);
	}

		
	public function form($instance){
                global $opt_val;
		$defaults = array( 'title' => $opt_val['ttpv_txt_recentpost_title'],
                                  'num_post' => $opt_val['ttpv_txt_recentpost_widget_number'],
                                  'authorslist' => implode(',', $opt_val['ttpv_drp_recentpost_widget_authors']),
                                  'displaytype' => $opt_val['ttpv_drp_recentpost_widget_postdisplay'],
                                  'categorylist' => implode(',', $opt_val['ttpv_drp_recentpost_widget_category'])
                                  );
		...
		?>
                ...
                <p>
                    ...
                    value="<?php if(isset($title)){
                                    ...
                                    echo esc_attr($opt_val['ttpv_txt_recentpost_title']);
                                }
                                ?>"  />
                </p>
			...
			<select id="<?php echo $this->get_field_id( 'displaytype' ); ?>" name="<?php echo $this->get_field_name( 'displaytype' ); ?>">
                ...
                <p>
                    ...
                    value="<?php if(isset($num_post)){
                                    ...
                                    echo esc_attr($opt_val['ttpv_txt_recentpost_widget_number']);
                                }
                                ?>"  />
                </p>
	...
	<?php	    
		...
		?>
		    ...
		    if($categories[0]=="Any" && $authors_array[0] == "Any"){
                        $parameters = array( 'posts_per_page' => $num_post, 'author' => '', 'orderby' => 'date', 'order' => 'DESC', 'ignore_sticky_posts' => 1 );
		    }
		    elseif($categories[0]!="Any" && $authors_array[0] == "Any"){
			$parameters = array( 'posts_per_page' => $num_post, 'category__in' => $catid, 'author' => '', 'orderby' => 'date', 'order' => 'DESC', 'ignore_sticky_posts' => 1 );
		    }
		    elseif($categories[0]=="Any" && $authors_array[0] != "Any"){
			$parameters = array( 'posts_per_page' => $num_post, 'author' => $authors_id, 'orderby' => 'date', 'order' => 'DESC', 'ignore_sticky_posts' => 1 );
		    }
                    else{			
                        $parameters = array( 'posts_per_page' => $num_post, 'category__in' => $catid, 'author' => $authors_id, 'orderby' => 'date', 'order' => 'DESC', 'ignore_sticky_posts' => 1 );
                    }
