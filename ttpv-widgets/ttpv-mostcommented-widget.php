...
add_action( 'widgets_init', 'initialize_mostcommented_widget' );
function initialize_mostcommented_widget() {
        global $opt_val;

        if($opt_val['ttpv_chk_mostcommented_widget'] == 'on'){
            register_widget( 'TTPV_MostCommented_Widget' );
        }
}

class TTPV_MostCommented_Widget extends WP_Widget {
	function __construct(){
	$options = array(
		'description' => "TTPV Widget: Display MostCommented Posts",
		'name' => 'TTPV: MostCommented Posts'
		);
	parent::__construct('TTPV_MostCommented_Widget','',$options);
	}


	public function form($instance){
                global $opt_val;
		$defaults = array( 'title' => $opt_val['ttpv_txt_mostcommented_title'],
                                  'num_post' => $opt_val['ttpv_txt_mostcommented_widget_number'],
                                  'authorslist' => implode(',', $opt_val['ttpv_drp_mostcommented_widget_authors']),
                                  'displaytype' => $opt_val['ttpv_drp_mostcommented_widget_postdisplay'],
                                  'categorylist' => implode(',', $opt_val['ttpv_drp_mostcommented_widget_category'])
                                  );
		...
		?>
                ...
                <p>
                    ...
                    value="<?php if(isset($title)){
                                    ...
                                    echo esc_attr($opt_val['ttpv_txt_mostcommented_title']);
                                }
                                ?>"  />
                </p>
			...
			<select id="<?php echo $this->get_field_id( 'displaytype' ); ?>" name="<?php echo $this->get_field_name( 'displaytype' ); ?>">
                ...
                <p>
                    ...
                    value="<?php if(isset($num_post)){
                                    ...
                                    echo esc_attr($opt_val['ttpv_txt_mostcommented_widget_number']);
                                }
                                ?>"  />
                </p>
                <?php	    
		...
		<ul>
			...
			//if(!comments_open( $mostcommented_posts->ID )){continue;}
			?>
